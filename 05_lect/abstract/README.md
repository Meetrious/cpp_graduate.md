...
>>    - `std::move` принимает левую ссылку в себя, приводит её к правой ссылке, возвращает последнюю. Правая ссылка без имени, как выражение - это *x-value*.
...
- c классом КЭШа та же история; он состоит из `std::list` и `std::unordered_map`. Он тоже не *RAII*. Нет нужды писать какие-либо нетривиальности, связанные с управлением памятью, самому. Внутри `std::list` и `std::unordered_map` уже всё написано до нас.
...
```c++
*(arr + 2*0 + 0) == *(arr + 0) // = 0b1100 = 12
*(arr + 2*0 + 1) == *(arr + 1) // = 0b1000 = 8
*(arr + 2*1 + 0) == *(arr + 2) // = 0b0011 = 3
*(arr + 2*1 + 1) == *(arr + 3) // = 0b1001 = 9
```
...
В *C/C++* так же существуют то, что называется *jagged arrays*. Массив из указателей. И он сам по себе не знает, сколько там элементов лежит под текущим указателем. И вот, например, мы ему сказали, что его указатели ссылаются на память уже существующего массива:
```c++
i4 arr[2][2];
i4 *jarr[2] = {arr, arr + 1};
```
...
Да, пусть наша модель начинается "с нуля". Первые 2 объекта по 5 бит - это указатели:
```c++
...
jarr + 1 == &(arr + 1);  // согласно инициализации
jarr[1] == *(jarr + 1) ==
        == адрес на объект, начинающийся с 18=(0b10010) бита = arr + 1;
...
Обычно `a[]` означает `*a` в точности почти всегда. Случай, когда это не так, следующий и по большей части связан с ассемблером, который генерируется в связи с тем, что было написано.
```c++
...
Поиграться с лин. алгеброй, свойствами определителя (конкретно с теми, что связаны с влиянием элементарных преобразований на его значение) и написать тесты для матриц 100x100.
