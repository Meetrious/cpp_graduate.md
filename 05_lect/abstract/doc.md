...
<br/> С учётом сказанного в конце предыдущей лекции можно уследить одно замечательное качество этого конструктора:
...
И замечательно это потому, что `ScopedPointer` гарантирует обратную совместимость с любым `legacy`-кодом, который оперировал сырыми указателями. Мы можем просто заменить все вхождения объявлений сырых указателей в нём на `ScopedPointer`-объекты и его логика сохранится.
