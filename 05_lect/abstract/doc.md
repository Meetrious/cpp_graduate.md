...
И здесь я на момент отклонюсь от конспектирования Владимирова и попытаюсь разъяснить релевантную содержательную часть [доклада Крис-Ван-Рена](https://www.youtube.com/live/km3Urog_wzk?feature=share) про категории выражений.
...
1. *The expression evaluates to an identity*. Можно понимать, как ***результат выражения имеет имя, или место в памяти***;
...
следующие выражения не имеют ни имени, ни места в памяти
  ...
  a++  /* если не присвоить, то на следующей строке значение изменится
        * а у текущего значения ~"как будто бы нет места в памяти". Текущее
        * значение а - временный объект в контексте этого выражения. 
        * Мне очень хочется сказать, что оно x-value и должно лежать выше. */
  ...
  a + 0   /* арифм. выражение которое не вычислится,
...
>>    - `std::move` принимает левую ссылку в себя, приводит её к правой ссылке.
...
>>
>> `std::move` "обманывает" *name-resolution-process* конструктора правой ссылки `rref`, и *l-value* `va` попадает на выгодные позиции в *overload-set* конструктора под видом *x-value*.
...
Таким образом, правилом хорошего тона звучит:
...
>>> !!! при этом очень важно, чтобы возвращаемый объект имел *scope* или *lifetime* ВНЕ ГРАНИЦ стека функции, возвращающей `type&&`. Иначе всё, конечно, скомпилируется, но будет провисание. 
...
int main() {
  ...
  * результат buz_1 - это *вау* предпочтительный кандидат для rref.operator= 
  * т.к. она всё ещё не связана с именем, значит материализовалась и
  * не является l-value. std::move при присваивании в rref не нужен. */

  int&& rref_2 = std::move(buz_2(va));  /* временный int&, коим и является
  * результат buz_2 - это невалидный кандидат для rref.operator=
...
>> }
>> ```
>> **Спойлер**: `move(`аргумент`)` никогда не свяжется с *non-const* левой ссылкой, а перегрузка с аргументом-значением будет конфликтовать со всеми остальными по отдельности. Однако ж если убрать перегрузку по значению; задефайнить <kbd>А</kbd>, то вырисовывается иерархия:
>> 1. `int&&`
>> 2. `const int &&`
>> 3. `const int &`
>>
...
>>
> **Упражнение**: чуть подкорректируйте код выше и выясните, с чем может связаться константная правая ссылка.
...
> 2. либо вы пишете *r-ref* аннотированный метод-член класса;
...
Помимо конструкторов по умолчанию, пользовательских и копирования, существуют ещё [**конструктор перемещения**](https://en.cppreference.com/w/cpp/language/move_constructor). 

Если у копирующего конструктора задача (семантика копирования): <div style="text-align: center"><b> скопировать правую часть в левую часть таким образом, чтобы оригинал и копия были взаимозаменяемы.</b></div><br/>
То задача перемещаюшего конструктора (семантика перемещения) более скромная: <div style="text-align: center"><b>передать владение ресурсами из правой части в левую часть, и оставить левую часть в констистентном, но необязательно предсказуемом состоянии. </b></div> <br/>
...
>>> *offtop*: **Константные *r-value* ссылки** - это очень редкий и на первый взгляд может показаться, что совершенно бесполезный объект. Как минимум с точки зрения семантики перемещения. Ведь конструктор перемещения, как минимум для RAII-класса, **должен мочь изменить** содержание `rhs` аргумента! Однако пара неожиданных тонкостей оперирования с методами, принимающими *const r-value-ref* есть. Например [вот](https://www.sandordargo.com/blog/2021/08/18/const-rvalue-references).
...
>> - потому что, 
>>    1. <u>если</u> мы хотим передать владение из одного объекта в другой, нужно вызвать конструктор перемещения.
>>
>>    2. По определению конструктор перемещения имеет аргумент *r-value* ссылку. А чтобы при создании нового объекта в *name-resolution-process* выиграл конструктор перемещения, нужно передать аргументом
...
Оно отличается от обычного присваивания (копирующего присваивания) тем, что, согласно семантике перемещения, правая сторона при перемещении остаётся в консистентном, но не обязательно предсказуемом состоянии. Опишем его для `ScopedPointer`
...
```
Можно сделать дешевле. Можно не заниматься освобождением ресурса, оставить это дело деструктору и просто обменять указатели. Непредсказуемость финального состояния правой части, которая диктуется семантикой перемещения, позволяет нам сэкономить на дорогой операции.
```c++
...
...
> **Вопрос**: что произойдёт, если вызывать `std::swap` для объектов типа без перемещаюшего конструктора и присваивания?
...
>> Можно сказать, что поэтому *default* *сopy*-присваивание и конструктор принимают именно <u>константную</u> левую ссылку.
...
```
Поскольку класс `std::vector<char>` перемещаемый и копируемый, то для `a` вызовется *copy*-конструктор от левой константной ссылки, на существующий в памяти *l-val*-объекта `obj`, вектора `data_`, а для `b` вызовется *move*-конструктор от правой ссылки, на временно материализовавшийся внутри *r-val*-объекта `X()`, вектора `data_`.
...
Узнав о `std::move` и о его возможностях, можно словить *`std::move`-fever*; that is компульсивно пихать `std::move` туда, куда не надо, в попытках быть выразительным. 

Не хочу ничего писать по этому поводу. В лекции всё достаточно прозрачно.
...
```
Здесь будет толи двойное, толи тройное удаление по одному и тому же адресу.

### [Правило пяти/нуля](https://youtu.be/_mLDaU4wSOo?t=3775)
Итак *rule of five*. Это правило хорошего тона при проектировании в ООП. И оно утверждает следующее:
...
Оно имеет место, поскольку при его нарушении включаются куда более сложные правила о том, когда они будут явно определены, когда неявно; в стандарте 2 страницы текста на этот счёт.

Короче, лучше не рисковать. *RAII*-класс требует нетривиального деструктора по своему определению, а потому, если мы его пишем, то для него НАДО определить все копирующие и перемещающие методы. В `SillyPointer` *правило пяти* нарушено, и на примере перемещений мы увидели цену за это нарушение.

И да, "удаление" - это один из вариантов нетривиального определения.

*Правило пяти* очень выгодно и полезно рассматривать с другим правилом хорошего тона из принципов *SOLID*. А именно с *SRP*(*Single Responsibility Principle*). 

Если наш класс управляет владением ресурса, и делает ещё что-то, то у этого класса **две ответственности**. И в контексте *SRP* это нехорошо. А потому формулируется ***правило нуля*** (*rule of zero*):

> *Если ваш класс требует нетривиального определения <u>хотя бы одного</u> из тех неявных пяти методов, и, таким образом, всех пяти,*
> 
> *то в нём **не должно быть никаких других методов**!*

Таким образом, принимая *правило нуля*, при проектировании сущностей для нас вещи становятся чуть проще: 
- либо наш класс управляет ресурсом,
- либо не требует нетривиальных деструктора, копирований, перемещений.
