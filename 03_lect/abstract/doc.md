...
Другая гарантия, которую даёт язык *C* - это то, что в *C* нет ничего неявного. Примером неявности в *C++* -- ссылки. Побитовое представление ссылки не определено/скрыто от программиста.
...
6. Если нет всех выше перечисленных, то последняя надежда на переменное число аргументов.  Определение с `var_arg`-ами, т.е. ф-циями с аргументами, где присутствуют чистые многоточия (эллипсис).
7. Если ко всем прочим и вараргов нет, то побеждают кандидаты, связь с которыми подразумевает неправильную связку передаваемых аргументов с ссылками. И, это не точно, но скорее всего ничем хорошим не заканчивается. В лучшем случае ошибка компиляции.
...
// переоткрытие пространсва имён для дополнения 
namespase MyNamespace {
...
2. Пространства имён можно переоткрывать сколько угодно раз, в отличие от структур и от классов. И вкладывать одни в другие на какую угодно глубину.
...
1. В глобальном поле, в отношении переменных или функций `static` - это internal linkage specifier.
...
>> Все сущности внутри анонимного `namespace` будут известны только внутри текущего модуля, ведь сгенерированное имя рандомное, уникальное, а главное, недостижимое, его нигде не посмотришь. То есть это *C++*-способ навесить *internal linkage* на все сущности внутри него без использования `static`-спецификатора линковки.
...
- **Не использовать анонимные пространства имён в заголовочных файлах.** Потому что генерация имени происходит один раз на единицу трансляции. То есть, если мы в несколько модулей подключим хедер с анонимным пространством имён, то на каждый из этих модулей будет понесёт на себе namespace с уникальным именем, но одними и теми же функциями, что существенно раздует код, бинарь...
