...
5. Если нет ни точных совпадений, ни шаблона, ни кандидатов на расстоянии в 1 standart implicit cast, но
    - есть совпадение с точностью до возможного пользовательского преобразования (или цепочки их цепочки) через перегруженные операторы у наших классов, то побеждает оно.
...
7. Если ко всем прочим и вараргов нет, то побеждают кандидаты, связь с которым подразумевает неправильную связку передаваемых аргументов с ссылками. И (это не точно но) скорее это ничем хорошим не заканчивается, в лучшем случае ошибка компиляции.
...
>> - "*Технический ответ следующий. Когда вы используете варарги, вы много платите. Когда вы используете точные типы, вы платите мало. Варарги же усложняют ABI. `va_list`, `va_start`, `va_arg` - с точки зрения ассемблера очень дорогие вызовы. Считается, что мы до последнего не хотим за этот вариант разрешения имени платить*"



## О пространствах имён и последние 2 назначения using
Очень много мы упоминали термин *пространства имён* выше, и сейчас в кои-то веки уточним.
