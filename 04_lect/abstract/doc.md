...
Короче, чтобы уйти от повторной инициализации, надо проинициализировать наши поля где-нибудь
```cpp
...
- *порядок инициализации полей списком определяется НЕ порядком перечисленных инициализаций в списке, а порядком расположения полей в классе.*
```cpp
struct Node {
