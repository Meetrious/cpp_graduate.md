...
Короче, чтобы уйти от повторной инициализации, а так же определить состояние для членов класса ссылок и const-указателей в объекте (если они есть), надо проинициализировать наши поля где-нибудь
```cpp
...
>> - при любом инстансе копирования дерева будут происходить плохие вещи; указатель на *top_, будет скопирован побитово из одного объекта типа `SearchTree` в другой объект типа `SearchTree`(ИМЕННО ЧТО УКАЗАТЕЛИ, а не данные, на которые они ссылаются), ведь тип указателя в *C/C++* - это, как ни крути, встроенный тип, не важно, во что он там разыменуется. А значит, по окончанию scope копии вызовется деструктор, и у оригинального дерева всё содержимое провиснет.
...
Синтаксис для "удаления/запрещения" базовых методов класса (конструкторов/операторов...), для копирования в частности, следующий:
```cpp
...
template <typename KeyT, typename Comp>
  class SearchTree {
   ...
   public:  // база
    ...
    SearchTree& operator=(const &SearchTree rhs) = delete;  // удаленный оператор=
...
Помимо невозможности присваивания значения одного существующего объекта другому, запрещение копирования в классе влечёт невозможность передать / вернуть объект этого класса по значению в / из функции, что, казалось бы, что-то само собой разумеющееся для любых объектов. Вообще говоря в стандарте, конечно, есть классы, для которых запрещено копирование (i.e. мьютекс). Но если здравый смысл вам подсказывает, что для текущего класса копирование в принципе должно быть возможно, пожалуйста, не злоупотребляйте `= delete;`
...
Поговорим о спец.семантике всего этого.
...
<div style="text-align: center"><b><i>Объект <u>this</u> после копирования эквивалентен ВО ВСЕХ отношениях тому объекту <u>rhs</u>, с которого копировали.</b></i></div>
...
Перечислим все эти формы. Они могут отличаться только конкретным диапазоном аннотаций к `foo&`-типу принимаемого аргумента rhs. И аргумент `rhs` может быть как угодно *cv-квалифицированной* ссылкой на `foo`. Имеется ввиду следующее.
```c++
struct Copyable {
  ...
...
Есть ключевое слово `const`, есть ключевое слово `volatile`.
```c
...
int S::foo() const volatile { return 42; }  /** означает, во-первых,
...
И по этой теме есть исторический анекдот