...
Оказывается, что вызов `static_cast` - это вызов того самого явного преобразования, от которого никакой `explicit` не защищает. Таким образом, если мы определяем конструктор с одним аргументом, или оператор приведения, то мы на самом деле определяем, как именно для соответствующих типов сработает `static_cast`.
...
---
z аналогично w  // т.к. результат тем более помещается в даже в signed ll
```
...
>>
>>А $2^{31} - 1 < 4 008 452 664$. Т.е. полученное число не влезает в положительный диапазон `int`-типа, а потому срежется по модулю $2^{31}$ и может стать станет отрицательным. В данном случае оно становится отрицательным. И без разницы, что результат сложения влезает в диапазон `unsigned long long`. 
>> 
...
> **offtop**: ***Кватернионы*** - это такие же сложно составные числа, как и комплексные, только в них чуть больше вариантов мнимых единиц (а именно 3), и определяются они не парой вещественных чисел, а четвёркой.
Множество всех таких чисел обозначается $\mathbb{H}$ в честь сэра Гамильтона, и определяется так:<br/>
>$$\mathbb{H} := \biggl\{ a + b \cdot i + c \cdot j + d \cdot k\quad : a,b,c,d \in \mathbb{R}\biggr\}$$
>
...
> 
> Развитие их теории было инспирированно задачами теоретической механики в связи с тем, что в их терминах очень легко описываются изометрические преобразования пространства (повороты).
...
Ну и действительно! У всех же есть этот маленький извечный соблазн определить предикат `==` для `double` в кои-то веки! И пофиг на погрешность машинной точности!
...
В случае переопределения `operator-` для `typename T` попытка invoke-нуть шаблонную перегрузку неявно будет тихо проигнорирована. Сработает механизм **SFINAE** (substitution failure is not an error), о котором мы поговорим во второй половине курса.
...
И она означает ровно то, что компилятор и пишет: "невозможно подставить `int` на место `T` в контексте шаблонного `operator-`". Ведь для `int` уже существует закреплённый оператор для навешивания знака (`-`).
