...
Рассмотрим его в контексте структуры кватернионов. 
```c++
...
>> - а в данном случае относительно без разницы.
...
> }  /* можно принимать аргумент и по значению, чтобы
> чтобы избавиться от неявных разыменований внутри реализации
> при обращении к полям кватерниона */
> ```
Можно заметить, что для варианта снаружи реализация и определение выглядят чуть более многословно, чем снаружи. Ну ок.

Интересно то, что он может быть определён и внутри, и снаружи одновременно.
...
У всех же есть тот маленький извечный соблазн определить предикат `==` для `double` в кои-то веки!
