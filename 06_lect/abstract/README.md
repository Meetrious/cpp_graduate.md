...
>>
>> Само собой, пока мы не умеем обрабатывать исключения (ничего не `try`-им, и ничего не ловим), выброс любого из них равносилен `std::terminate` вне зависимости от того, аннотированна ли функция `noexcept` или нет.

Пожалуй, если внутри реализации нашей функции вызываются только `noexcept`-функции, то нашу функцию тоже можно аннотировать `noexcept`. Логично и обратное; если наша функция `noexcept`, то внутри её реализации не должно быть вызовов функций, которые могут бросить исключение.
...
Одна из реальных ситуаций, в которой `noexcept`-аннотация безопасна - это правильно реализованные перемещающий конструктор и перемещающее присваивание в *RAII*-классах, ведь эти методы лишь обменивают значения между указателями. Обмен значениями указателей - это примитивная операция над примитивными типами.
...
<br/> является очень плодотворной, когда нам, например, нужно описать цепочку операторов для доступа в заинкапсулированные данные. Полезна она не только в этом примере, и выручит нас ещё не раз.
...
- оператор приведения к другому типу с одним аргументом 
...
В `unique_ptr` в приватном поле стоит `T* ptr_;`, в который можно завернуть любой ресурс (`FILE`, поток, память).
> **Возникает закономерный вопрос**: если, а откуда `unique_ptr` знает, как удалять, если во всех трёх случаях ресурс освобождается по-разному?
...
*RAII*-обёртка `std::unique_ptr` имеет привычный нам интерфейс указателя, за одним исключением:
...
Дело в том, что сигнатура перегрузки функции `make_unique`, которая выигрывает в данном случае, выглядит так:
...
Это инстанс **вариативно-шаблонной магии**, о которой мы пока вообще не готовы говорить. До неё мы дойдём и обучимся второй половине курса.
...
Ненормативные средства C-style кастов и интерфейса сырых указателей, с помощью которых можно сделать всё что угодно, у нас никто отобрать в *C++* никогда не сможет.
...
- типы могут иметь ассоциированное поведение (наверное речь о том, что у пользовательских типов есть виртуальные методы, реализация которого зависит от динамического типа объекта)
...
С другой - любой объект какого угодно типа - это кусок памяти в линейной парадигме целевой архитектуры. И даже в *C++*, если мы напишем что-то такое.
```c++
#include <iostream>
...
int main () {
   ...
   * Отступаем 16 бит (2 байта) справа и действительно 1000 0000 = 0x80 */
}
```
...
1. Приведение из одного типа в другой с сохранением семантического значения. Оно, вероятно, может повлечь изменение побитового представления. <br/><br/>Таким, например, является приведение `int` к `float`. Побитовые представления объектов этих типов очень сильно отличаются, даже если их семантические значения совпадают.
> ```c++
...
2. Приведение снятия константности и волатильности. Например приведение `const int*` к `int*`. Опасно, т.к. нарушает обещания, но порой без этого никуда. Попробуйте функции из `string.h` реализовать на *C* без возможности снять константность с аргументов. Оно не подразумевает изменения побитового представления.
...
Каждый из трёх `cast`-ов ограничен в контекте трёх приведённых выше характеров. То есть
-  `static_cast` сохраняет семантику, но не способен снять cv-квалификацию или вслепую реинтерпретировать биты;
...
> - не является связанным приводимой (с помощью `static_cast`) цепочкой наследования к типу `A` (или, если короче, не является вариацией динамического типа `A`)
...
Оказывается, что вызов `static_cast` - это вызов того самого явного преобразования, от которого никакой `explicit` не защищает. Таким образом, если мы определяем конструктор с одним аргументом, или оператор приведения, то мы, на самом деле, в частности, определяем, как именно для соответствующих типов сработает `static_cast`.
...
```
Copy-инициализации это тоже касается. Напомним, что `explicit` её блокирует. Но `static_cast` по барабану на `explicit`.
...
Пусть `y` - это нечто между `int` включительно, и всем, что больше него. Рассмотрим следующие приведения.
```c++
int a = (int)y;  // C-style-cast из C
int b = int(y);  // functional c-style-cast из C++
```
Второго в *C* нет. Бытует мнение, что во второй строчке происходит вызов некоего мифического конструктора для `int`. Но на самом деле это не так. Это просто *c-style-cast* вывернутый наизнанку, пусть и напоминает синтаксис конструктора. Бывает ещё такое:
```c++
int c = int{y};
```
...
## [Особенности неявного приведения примитивных типов в *C*.<br/> *Integral promotion*. ](https://youtu.be/LDoCQzSd3xo?t=3223)

В *С++* от его предтечей нам достались неявные арифметические преобразования. Подметим на примерах некоторые странности с ними связанные.
```c++
int a = 2; double b = 2.8;
short c = a * b;  // с = ?
```
> **Вопрос**: чему равен c?
>> - `c = 5`, ведь `(short)5.6` округляется в сторону нуля.

И да, это так. Тут вроде бы всё понятно. 

Теперь пример более неочевидный. 

Вспомним, что `short` - это 16-ти битный целочисленный тип. 16 бит = 4 бинарной тетрады = 4 hex-числа. Пусть:
```c++
unsigned short x = 0xFFFE, y = 0xEEEE;  // x = 65534, y = 61166
 // x * y = 0xEEEC2224 = 4 008 452 664 в точности.
```
> **Вопрос**: какой результат будет помещён в следующие переменные после перемножения `x` и `y`?
> ```c++
> unsigned short v = x * y;  // v = ?
> unsigned w = x * y;  // w = ?
> unsigned long long z = x * y;  // z = ? 
> ```
И интуиция подсказывает, что:
```c++
v = 0x2224  // т.к. при переполнении результат срежется
---
тип w = unsigned = unsigned int  // 4 байта. Результат поместится.
w = 0x00002224  // на этапе исполнения операции результат срежется, или
w = 0xEEEC2224  // не срежется
---
z аналогично w  // т.к. результат тем более помещается даже в signed ll
```
С первым ожидания не подводят. Со вторым - частично. А с третьим - [увы, увы](https://github.com/tilir/cpp-graduate/blob/master/05-opers/intprom.cc).

<img src="imgs/intprom.png"> <br/>

> **Вопрос**: Почему так?

Секрет состоит в [**правилах численных продвижений**](https://en.cppreference.com/w/cpp/language/implicit_conversion) для бинарных операций.

Перечислим их по порядку.

Применять сверху вниз:

>> **1. `fp_type` op `type` $\Longrightarrow$ `fp_type` op `fp_type`**
> - Любой `type`-тип при смешении с типом-с-плавающей-точкой (*that is* `fp_type`) в бинарной операции сначала неявно приводится соответствующему типу-с-плавающей-точкой, и только потом осуществляется операция между двумя `fp_type`-объектами.
>
> При смешении двух плавучих типов превалирует наибольший. То есть: 
> ```c
> long double 'op' double ==> long double 'op' long double`
> ```
> ---
> 
>> **2. `type` op `u_itype` $\Longrightarrow$ `u_itype` op `u_itype`**
> - Здесь логика та же, только `type` - произвольный тип без плавающей точки, `itype` -  любой из {`long long`, `long`, `int`} вариантов типов. `u_`-приставка стоит за модификатор `unsigned`. 
>
 Предположу (я этого не проверил), что если `type` совпал с одним из `itype`-вариантов, то всё неявно приводится к `unsigned`-аналогу наибольшего присутствующего `itype`. То есть.
 ```c
 long long 'op' unsigned int ==> unsigned ll 'op' unsigned ll
 ```
> 
> ---
>> **3. `type` op `itype` $\Longrightarrow$ `itype` op `itype`**
> - Здесь всё аналогично предыдущему, только `type` нельзя иметь плавающую точку и быть `u_itype` вариантом.
> ---
>> **4. `itype_less_then_int` op `itype_less_then_int` $\Longrightarrow$ `int` op `int`**
> - Здесь `itype_less_then_int` - это любой из `signed`/`unsigned` аналогов {`short`, `char`}

Последнее правило объясняет, почему `z = 0xFFFFFFFFEEEC2224`, а не `0x00000000EEEC2224`.

Ответ следующий:
>> - `x`, `y` имеют тип `unsigned short`; этот тип меньше `int`, а значит по **4му** правилу:
>> 1. `x`, `у` неявно приведутся к `int`
>> 2. Перемножатся в значение `0xEEEC2224`
>>
>> Такое дело; `0xEEEC2224` = $4 008 452 664$. И увы, но диапазон 4-х-байтного `int`-типа - это $[-2^{31}, 2^{31} - 1]$. 
>>
>>А $2^{31} - 1 < 4 008 452 664$. Т.е. полученное число не влезает в положительный диапазон `int`-типа, а потому срежется по модулю $2^{31}$ и может стать отрицательным. В данном случае оно становится отрицательным. И без разницы, что результат сложения влезает в диапазон `unsigned long long`. 
>> 
>> 3. Результат не пролез в положительный диапазон `int`, стал отрицательным.
>> 4. По инициализации переменной `z` результат привёлся к `unsigned long long`, как "отрицательное", а скорее, как очень и очень большое положительное число, кое и началось с ffff... И с оригиналом его связывает только 16-тиричный хвост.
>
> **`0x FFFF FFFF EEEC 2224` = 18 446 744 073 423 036 964**

Совершенно не очевидная дичь. Но теперь, кажется, всё прозрачно.

> **Вопрос**: Откуда взялась такая странная схема неявного приведения?
>> Оказывается, правила [floating-point and integral promotion](https://en.cppreference.com/w/cpp/language/implicit_conversion) написаны после гиганского кол-ва экспериментов с производительностью и оттачивались годами. В целом они работают через 4 означенных пункта, которые надо просто запомнить.

На этапах инициализации, в частности при передачи аргументов по значению, происходят аналогичные неявные преобразования с примерно той же логикой.

--- 

Вот мы и готовы перейти к перегрузке операторов

# Перегрузка операторов 
Начнём с очень интересного случая оператора, который вообще лучше не перегружать.

## [Унарный плюс (positive hack)](https://youtu.be/LDoCQzSd3xo?t=3896)

Оператор унарного плюса почти для всех встроенных типов не значит ничего. В арифметических операциях над численными типами он ничего не делает. `2 == +2`

Но для пользовательских типов, в не перегурженном состоянии, у него появляется очень любопытное свойство.

Пусть, к примеру, мы имеем:

```c++
// тип с приведением из себя в long, возвращающий 42 от любого объекта
struct Foo{ operator long() { return 42; } };

// и две перегрузки функции bar
void bar(int x);
void bar(Foo x);
```
> <div style="text-align: center"><b>Не перегруженный унарный <code>operator+</code> применённый к объекту пользовательского типа умеет создавать контекст, в котором будет уместно неявное приведение объекта к численному типу.</b></div>

<br/>

В нашем случае у `Foo` типа есть оператор приведения к `long`. Таким образом:

```c++
int main() {
  Foo f;
  bar(f);  // Выиграет bar(Foo) т.к. точное совпадение.
  bar(+f)  // + при f создаст контекст для (long)f. Выиграет bar(int).
}
```
Вот это неявное приведение типа через не перегруженный `operator+` и называется *positive hack*-ом.
> **Вопрос**: что будет в этой ситуации, если в структуре определены операторы приведения к нескольким численным типам?

Чёрт его знает, скорее всего неоднозначность; в таком случае деваться по тем или иным причинам будет некуда, и придётся вызывать `static_cast<`*к конкретному типу*`>`. Но в данном простом случае, когда приведение к численному типу всего одно (как оно чаще всего и бывает), `static_cast` - это чуть более многословно, чем хотелось бы. 

В связи с означенным финтом, унарный `operator+` никогда не перегружают, ведь способность <u>создать контекст к численному приведению</u> ценой такого немногословного кода - это суперспособность, которую не воспроизвести никакой перегрузкой.

      - унарный operator+ и positive hack нам встретятся в будущем, когда дойдём до lambda-выражений.

---

После `operator+` логично рассматривать `operator-`.
## [Унарный минус. Перегрузка внутри/снаружи.](https://youtu.be/LDoCQzSd3xo?t=4115)

Рассмотрим его в контексте структуры кватерниона. 
```c++
template<typename T> struct Quat {
  T a, b, c, d;
};  // агрегатненько
```
Уточним, про что эта структура. 
> **offtop**: [***Кватернионы***](https://habr.com/ru/articles/426863/) - это такие же сложно составные числа, как и комплексные, только в них чуть больше вариантов мнимых единиц (а именно 3), и определяются они не парой вещественных чисел, а четвёркой.
Множество всех таких чисел обозначается $\mathbb{H}$ в честь сэра Гамильтона, и определяется так:<br/>
>$$\mathbb{H} := \{ a + b \cdot i + c \cdot j + d \cdot k\quad : a,b,c,d \in \mathbb{R}\}$$
>
> Как и множества вещественных и комплексных чисел, кватернионы так же замкнуты относительно сложения, умножения и даже деления. Операции (`-`), (`+`), (`*`), (`/`) определены самым естественным образом. Только вот в кватернионах умножение не перестановочно. Другими словами, $\mathbb{H}$ - это "**некоммутативное кольцо с делением**", или "**тело**". 
> 
> Развитие их теории было инспирировано задачами теоретической механики в связи с тем, что в их терминах очень легко описываются изометрические преобразования (повороты).

Как мы знаем, в таком виде структуры операции копирование и присваивание уже неявно определены. Для неё также можно определить и сложение, умножение, деление. Но начнём с простого и определим унарный `operator-`, который просто проносит знак "-" в кватернион.

`operator-` может быть определён как внутри класса, так и снаружи; 
- внутри, как метод без аргументов;
- снаружи, как функция с одним аргументом.

> **Вопрос**: где его корректно определить?
>> - а в данном случае относительно без разницы. Но отличия есть.
...
> };
> 
> // реализацию можно сделать внутри тела, 
> // а можно вынести её в отдельный модуль
> template<typename T>
...
>   return Quat<T>{-arg.a, -arg.b, -arg.c, -arg.d};
> }  /* можно принимать аргумент и по значению,
> чтобы избавиться от неявных разыменований внутри реализации
> при обращении к полям кватерниона */
> ```
Можно заметить, что для варианта снаружи, реализация выглядит чуть более многословно, чем внутри. Ну ок.

Интересно то, что `operator-` может быть определён и внутри, и снаружи, одновременно.
> **Вопрос**: какой победит в сокращенном вызове `-q`, если будут определены оба?
> т.е. в ситуации:
> ```c++
>int main() {
>  Quat<float> q{1, 2, 3, 4};
>  Quat<float> p = -q;  // здесь вызовется внешний, или внутренний?
>  Quat<float> r = operator-(q);  // явный вызов внешнего
>  Quat<float> s = q.operator-();  // явный вызов внутреннего
>}
>```
>> - победит тот, что внутри. Можно [попробовать и убедиться](https://github.com/tilir/cpp-graduate/blob/master/05-opers/twovariants.cc).

> **Вопрос**: а почему так?
>> - потому что оператор, описанный внутри класса, может быть описан разработчиком этого класса, и только им. А тот, что описан снаружи, может быть добавлен кем угодно. И в случае, когда оператор перегружен и внутри, и снаружи - это две функции из разных пространств имён, т.е. это не *ambiguity*; при сокращенном вызове приоритет за версией автора структуры.

---

Круто. Мы только что определили смысл символа арифметической операций для нашего типа. Наш следующий закономерный соблазн - повлиять на смысл арифметической операции для чего угодно. 

Ну и действительно! У всех же есть этот маленький извечный соблазн определить предикат `==` для `double` в кои-то веки! И пофиг на погрешность машинной точности в вычислениях, результат которых помещён в аргументы предиката!

К примеру, переопределим унарный минус для `int`.

```c++
int operator-(int x) {
  std::cout << "MINUS!" << std::endl;
  return x;  // да, он ничего не делает, а только стреляет в stdout
}
```

а если не сработает, то перегрузим `operator-` для всего подряд, в том числе и для `int`:
```c++
...
```
В обоих случаях, к сожалению, или счастью, нас ждёт либо игнор, либо ошибка компиляции. [Feel free to try it yourself](https://github.com/tilir/cpp-graduate/blob/master/05-opers/builtin.cc) (флаги `-DCERR`, `-DCERR2` при компиляции в помощь)
...
В первом случае (в случае переопределения минуса целенаправленно для `int`):

<img src="imgs/op_min_for_int.png"> <br/>
Ошибка компиляции вежливо подсказывает нам, мол, "вы, должно быть, перепутали, и имели ввиду `operator-` для `enum`". Т.к. для перечисляющих типов мы таки можем переопределить `operator-`.

В случае переопределения `operator-` для `typename T` попытка invoke-нуть шаблонную перегрузку неявно будет тихо проигнорирована и в `stdout` выстрелит "-42". Сработает механизм **SFINAE** (*substitution failure is not an error*), о котором мы поговорим во второй половине курса.

Но если мы попросим шаблонный `operator-` явно для `int`, то мы получим **ошибку подстановки**.
...
```
<img src="imgs/temp_op_min_for_int.png"> <br/>

И она означает ровно то, что компилятор и пишет: "неудача при дедуцировании/подстановки переменной в шаблонный аргумент". Невозможно подставить `int` на место `T` в контексте шаблонного `operator-`. Ведь для `int` уже существует, закреплённый авторами языка, оператор для навешивания знака (`-`).

> **Вывод**: унарный минус - весьма сомнительный оператор для шаблонной перегрузки.
