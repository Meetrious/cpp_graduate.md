...
1. Приведение из одного типа в другой с сохранением семантического значения. Оно, вероятно, может повлечь изменение побитового представления. <br/><br/>Таким, например, является приведение `int` к `float`. Побитовые представления объектов этих типов очень сильно отличаются, даже если их десятичные значения совпадают.
> ```c++
...
> - Здесь `itype_less_then_int` - любой из `signed`/`unsigned` аналогов {`short`, `char`}

Последнее правило объясняет, почему `z = 0xFFFFFFFFEEEC2224`, а не `0x00000000EEEC2224`. Ответ следующий:
...
Мы готовы начать тему перегрузки операторов.
...
Чёрт его знает, что бы было тогда; в таком случае деваться по тем или иным причинам было бы некуда, и пришлось бы вызывать `static_cast<`*к конкретному типу*`>`. Но в данном простом случае, когда приведение к численному типу всего одно, `static_cast` - это чуть более многословно, чем хотелось бы. 
...
В связи с означенным финтом, унарный `operator+` никогда не перегружают, ведь способность <u>создать контекст к численному приведению</u> ценой такого немногословного кода - это суперспособность, которую не воспроизвести никакой перегрузкой.
      ...
      - унарный operator+ нам встретится в будущем, когда дойдём до lambda-выражений.
...
> **offtop**: **Кватернионы** - это такие же сложно составные числа, как и комплексные, только в них чуть больше вариантов мнимых единиц, а именно 3.
Выглядят эти числа вот так  
И в отличие от вещественных и комплексных, для кватернионов не определена операция деления. 
