...
В `unique_ptr` в приватном поле стоит `T* ptr_;`, в который можно завернуть любой ресурс(FILE, поток, переменная типа). Возникает
> **закономерный вопрос**: если, а откуда `unique_ptr` знает, как удалять, если во всех трёх случаях ресурс освобождается по-разному?
...
```
Копирующей инициализации это тоже касается. Напомним, что `explicit` её блокирует. Но `static_cast` по барабану на `explicit`.
...
```
> **Вопрос**: какой результат будет помещён в следующие переменные после их перемножения?
>```c++
...
тип w = unsigned = unsigned int  // результат должен поместиться в int
w = 0xEEEC2224  // на этапе исполнения операции результат срежется
или же
w = 0x00002224  // иначе
z аналогично w  // т.к. результат тем более помещается в long long
```
...
>
> Если `type` совпал с одним из `itype`-вариантов, то всё неявно приводится к `unsigned`-аналогу наибольшего присутствующего `itype`. То есть.
> ```c
...
>> - `x`, `y` имеют тип `unsigned short`. А значит по **4му** правилу:
...
>>
>>А $2^{31} - 1 < 4 008 452 664$. Т.е. полученное число не влезает в положительный диапазон `int`-типа, а потому срежется по модулю $2^{31}$ и может стать станет отрицательным. В данном случае оно становится отрицательным И без разницы, что в полученное число влезет в диапазон типа `unsigned long long`. 
>> 
>> 3. Результат не пролез в положительный диапазон `int`, стал отрицательным.
>> 4. По инициализации переменной `z` результат привёлся к `unsigned long long`, как "отрицательное число", кое и началось с ffff...

Совершенно не очевидная дичь. Какому больному!?...кхм...

> **Вопрос**: Откуда взялась такая странная схема неявного приведения?
>> Оказывается, правила [floating-point and integral promotion](https://en.cppreference.com/w/cpp/language/implicit_conversion) написаны после гиганского кол-ва экспериментов с производительностью и оттачивались годами. В целом они работают через означенные 4 правила, которые надо просто запомнить.